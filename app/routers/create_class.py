...
def create_posts(data: schemas.CreateNewClass, user_data = Depends(oauth2.get_current_user)):
    ...
    cursor.execute("""SELECT week_id as max_week_id FROM join_tutor_week ORDER BY max_week_id desc LIMIT 1""", []) # INIT: trzeba będzie dodać pierwszy id
    max_week_id = cursor.fetchone()
    max_week_id = int(max_week_id['max_week_id']) + 1

    for day in days:
        cursor.execute("""SELECT MAX(schedule_id) as max_schedule_id FROM schedule_array""") # INIT: trzeba będzie dodać pierwszy id
